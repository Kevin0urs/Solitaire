#!/usr/bin/env python3
"""
CLI Test Suite and Local Web Server for Spider Solitaire - Solitaire de Mamie Nicole
"""
...
class SpiderGameEngineTest(unittest.TestCase):
    def test_deck_composition_1suit(self):
        cards = []
        for set_idx in range(8):
            for rank in range(1, 14):
                cards.append(Card('spades', rank))
        
        self.assertEqual(len(cards), 104, "104 cards total")
        spades = [c for c in cards if c.suit == 'spades']
        self.assertEqual(len(spades), 104, "104 spades in 1-Suit mode")

    def test_deck_composition_2suits(self):
        cards = []
...
def run_tests():
    ...
    print("=" * 60)
    print(" [TESTS] SOLITAIRE DE MAMIE NICOLE - PYTHON VALIDATION SUITE")
    print("=" * 60)
